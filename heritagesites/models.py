...
class HeritageSite(models.Model):
    ...
    def country_area_display(self):
        ...
        return ', '.join(country_area.country_area_name for country_area in self.country_area.all()[:25])
...
class HeritageSiteJurisdiction(models.Model):
	...
	# entries will be deleted if corresponding parent record in the heritage_site
    # or country_area table is deleted. This mirrors CONSTRAINT behavior in the
    # MySQL back-end.
...
class Location(models.Model):
    ...
    class Meta:
        ...
        verbose_name = 'UNSD M49 Location'
        verbose_name_plural = 'UNSD M49 Locations'

    def __str__(self):
        location_combo = "{} - {} - {} - {}".format(self.planet, self.region, self.sub_region, self.intermediate_region)
        return location_combo
